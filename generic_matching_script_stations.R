...
#Then read in the shape file that outlines your geographic units (neighborhoods, census tracts, etc...)
geo_data <- read_sf("data/geo_export_c40bd568-a9cc-41fc-bc11-4e4d7bc37581.shp")
...
map_point_to_geo <- function(point_data, geo_data, name10, assign_if_not_in, long_lat_vec){
  ...
  point_data <- point_data_sf %>% 
    mutate(intersection = as.integer(st_intersects(geometry, geo_data)),
           name10 = if_else(is.na(intersection), '', geo_data[[name10]][intersection])) 
  
  point_data_done <- tibble(point_data) %>%
    mutate(name10 = ifelse(is.na(intersection), assign_if_not_in, name10)) %>%
    dplyr::select(-intersection)
...
census_data <- map_point_to_geo(point_data, geo_data, "name10", "None", c("longitude", "latitude"))

View(census_data)

census_data <- census_data %>% 
  mutate(census_tract = name10) %>% 
  dplyr::select(-name10)

# merge to get list of stations with community and census tract and lattitude longitude

write_csv(census_data, "data/processed/census_tract.csv")
